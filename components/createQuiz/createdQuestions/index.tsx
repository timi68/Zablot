...
import {Fab, Button, Box, Tooltip} from "@mui/material";
	...
	forwardRef(
		(
			...
			useImperativeHandle(
				...
				() => ({
					setQuestions(
						newQuestion: Interfaces.Question,
						questionId?: number
					) {
						if (questionId !== undefined) {
							setState(
								(
									oldState: Interfaces.State
								): Interfaces.State => {
						...
						setState((oldState) => {
							...
							Cookie.set(
								"Questions",
								JSON.stringify(newQuestions.questions)
							);
			...
			useLayoutEffect(() => {
				if (state.questions?.length) {
					showSnackbar(
						"You have unfinished quiz set up",
						"info",
						4000
					);
				}
			...
			useLayoutEffect(() => {
				if (!isNaN(state.update)) {
					let scrollToView =
						(
							questionListRef.current.children[
								state.update
							] as HTMLLIElement
						).offsetTop - 70;
